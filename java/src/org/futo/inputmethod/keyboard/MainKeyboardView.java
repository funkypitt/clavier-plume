...
public final class MainKeyboardView extends KeyboardView implements DrawingProxy,
    ...
    private void showKeyPreview(@Nonnull final Key key) {
        ...
        getLocationInWindow(mOriginCoords);
        // Plume : l'aperçu apparaît instantanément, comme sur iOS (pas d'animation de zoom)
        mKeyPreviewChoreographer.placeAndShowKeyPreview(key, keyboard.mIconsSet, getKeyDrawParams(),
                getWidth(), mOriginCoords, mDrawingPreviewPlacerView, false /* withAnimation */,
    ...
    // Plume : fondu des étiquettes en mode trackpad (150 ms, comme iOS)
    private android.animation.ValueAnimator mPlumeFadeAnimator;
    @Override
    public void onPlumeTrackpadMode(final boolean on) {
        if (mPlumeFadeAnimator != null) mPlumeFadeAnimator.cancel();
        final float target = on ? 0.0f : 1.0f;
        mPlumeFadeAnimator = android.animation.ValueAnimator.ofFloat(sPlumeLabelAlpha, target);
        mPlumeFadeAnimator.setDuration(150);
        mPlumeFadeAnimator.addUpdateListener(a -> {
            sPlumeLabelAlpha = (float) a.getAnimatedValue();
            invalidateAllKeys();
        });
        mPlumeFadeAnimator.start();
    }
    ...
    private void dismissKeyPreview(@Nonnull final Key key) {
        // Plume : disparition immédiate au relâchement (iOS), sans fondu
