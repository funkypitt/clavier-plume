...
public class KeyDetector {
    ...
    /** Plume : poids 0..1 par code point (probabilité de la lettre suivante). */
    @Nullable
    private java.util.Map<Integer, Float> mBoostWeights;
    private static final float PLUME_DEFAULT_WEIGHT = 0.35f;
    ...
    public Key detectHitKey(final int x, final int y) {
        ...
        final boolean hasBoost = (mBoostedCodePoints != null && !mBoostedCodePoints.isEmpty())
                || (mBoostWeights != null && !mBoostWeights.isEmpty());
        if(hasBoost) {
            for (final Key key : mKeyboard.getNearestKeys(touchX, touchY)) {
                final float w = plumeBoostWeight(key.getCode());
                if (w > 0f) {
                    // Plume : zone de frappe invisible proportionnelle à la probabilité de la
                    // lettre (iOS : A_total = A_visible + A_hidden × P). Une lettre certaine
                    // déborde jusqu'à la moitié des touches voisines, une lettre improbable
                    // presque pas. La distance est aussi pondérée pour départager.
                    final float distance = key.distanceToEdge(touchX, touchY);
                    final float reach = Math.min(key.getWidth(), key.getHeight()) * (0.15f + 0.35f * w);
                    if(distance > reach)
                        continue;
                    final float metric = distance / (0.5f + w);
                    if(metric < minDistance) {
                        minDistance = metric;
                        primaryKey = key;
                    }
                }
    ...
    public void updateBoostedCodePoints(@Nullable Set<Integer> validNextCodePoints) {
        ...
        mBoostWeights = null;   // nouvelle frappe : les poids des suggestions précédentes sont périmés
    }

    /** Plume : poids probabilistes calculés depuis les suggestions (voir PlumeKeyBoost). */
    public void updateBoostWeights(@Nullable java.util.Map<Integer, Float> weights) {
        mBoostWeights = weights;
    }

    private float plumeBoostWeight(final int code) {
        float w = 0f;
        if (mBoostWeights != null) {
            final Float v = mBoostWeights.get(code);
            if (v != null) w = v;
        }
        if (mBoostedCodePoints != null && mBoostedCodePoints.contains(code)) {
            w = Math.max(w, PLUME_DEFAULT_WEIGHT);
        }
        return w;
    }
}
