...
public final class KeyPreviewChoreographer {
    ...
    private void placeKeyPreview(final Key key, final KeyPreviewView keyPreviewView,
        ...
        // Plume : ballon iOS plus large que la touche (× 1,7), col vers la touche, glyphe plus grand
        final boolean balloon = keyPreviewView.getPlumeBalloon() != null;
        final int previewWidth = balloon
                ? Math.min(Math.max((int)(key.getWidth() * 1.7f), (int)(44.0f * density)), (int)(72.0f * density))
                : Math.min(Math.max(key.getWidth(), (int)(34.0f * density)), (int)(64.0f * density));
        int previewHeight = topArea + bottomPadding;
        if (balloon) {
            final org.futo.inputmethod.latin.plume.PlumeBalloonDrawable b = keyPreviewView.getPlumeBalloon();
            b.setColWidthPx(key.getDrawWidth());
            b.setTopHeightPx(topArea);
            keyPreviewView.setTextSize(TypedValue.COMPLEX_UNIT_PX, key.selectTextSize(drawParams) * 1.3f);
        }
