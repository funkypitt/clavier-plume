...
class BasicThemeProvider(val context: Context, val colorScheme: KeyboardColorScheme) :
    ...
    /**
     * Plume : touche avec ombre portée de 1 dp vers le bas (iOS : noir 30 % en clair,
     * 70 % en sombre), sans flou. Les touches transparentes (sans bordure) n'en ont pas.
     */
    /** Plume Encre : couleur du contour des touches (null = pas de contour). */
    private var plumeOutlineColor: Int? = null

    private fun shadowedRoundedRectangle(@ColorInt color: Int, radius: Float): Drawable {
        if (Color(color).alpha < 0.05f) return coloredRoundedRectangle(color, radius)
        plumeOutlineColor?.let { outline ->
            return coloredRoundedRectangle(color, radius).apply {
                setStroke(dp(1.5.dp).toInt().coerceAtLeast(2), outline)
            }
        }
        val dark = Color(color).luminance() < 0.35f
        val shadowColor = Color(0f, 0f, 0f, if (dark) 0.7f else 0.3f).toArgb()
        val offset = dp(1.dp).toInt().coerceAtLeast(1)
        val layers = LayerDrawable(arrayOf(
            coloredRoundedRectangle(shadowColor, radius),
            coloredRoundedRectangle(color, radius)
        ))
        layers.setLayerInset(0, 0, offset, 0, 0)   // ombre : décalée vers le bas
        layers.setLayerInset(1, 0, 0, 0, offset)   // touche : laisse voir l'ombre en bas
        return layers
    }
    ...
    private fun makeVisualStyle(background: Int, foreground: Int, highlight: Int, foregroundPressed: Int, roundedness: Dp): VisualStyleDescriptor {
        val bg = shadowedRoundedRectangle(background, dp(roundedness))
        val bgPressed = shadowedRoundedRectangle(Color(highlight).compositeOver(Color(background)).toArgb(), dp(roundedness))
    ...
    init {
        ...
        if (advanced.themeName?.startsWith("Plume Encre") == true) {
            plumeOutlineColor = colorScheme.onKeyboardContainer.toArgb()
        }
        ...
        // Plume : couleur des glyphes des touches de fonction (palette : onSecondaryContainer),
        // nécessaire quand les fonctions sont d'une autre couleur que les lettres (Encre inversée)
        val plumeFuncText = if (advanced.themeName?.startsWith("Plume") == true)
            colorScheme.onSecondaryContainer.toArgb() else onKeyColor
        ...
        // Plume : rayon 5 dp (iOS) au lieu de 9 dp
        val keyCornerRadius = 5.dp * roundness
        ...
        // Plume : touche Retour rectangulaire arrondie comme les autres (iOS), pas une pilule
        val actionKeyRadius = keyCornerRadius

        var stylesTmp: Map<KeyVisualStyle, VisualStyleDescriptor> = mapOf(
            ...
            KeyVisualStyle.Functional to if(keyBorders) {
                makeVisualStyle(
                    functionalKeyColor,
                    if(expertMode) Color(onKeyColor).copy(alpha = 0.2f).toArgb() else plumeFuncText,
                    highlight, highlightForeground,
                    keyCornerRadius
                )
            } else {
        ...
        if (advanced.themeName?.startsWith("Plume") == true) {
            // iOS : une lettre pressée ne change pas (le ballon fait le travail) ; une touche de
            // fonction pressée prend la couleur des lettres (grise → blanche en clair).
            val letterBg = shadowedRoundedRectangle(keyColor, dp(keyCornerRadius))
            stylesTmp = stylesTmp.toMutableMap().apply {
                for (st in listOf(KeyVisualStyle.Normal, KeyVisualStyle.Functional, KeyVisualStyle.StickyOff)) {
                    this[st]?.let { put(st, it.copy(
                        backgroundDrawablePressed = letterBg,
                        foregroundColorPressed = it.foregroundColor,
                        // pas d'indicateur de « flick » directionnel : même fond pressé partout
                        backgroundDrawableFlicking = it.backgroundDrawableFlicking?.mapValues { letterBg })) }
                }
            }
        }
        keyStyles = stylesTmp
        ...
        val isPlume = advanced.themeName?.startsWith("Plume") == true
        keyFeedback = if (isPlume) {
            // Plume : aperçu « ballon » iOS, couleur de la touche, ombre 1 dp
            val dark = colorScheme.keyboardContainer.luminance() < 0.35f
            KeyBackground(
                foregroundColor = colorScheme.onKeyboardContainer.toArgb(),
                outlineColor = null,
                background = org.futo.inputmethod.latin.plume.PlumeBalloonDrawable(
                    colorScheme.keyboardContainer.toArgb(),
                    Color(0f, 0f, 0f, if (dark) 0.7f else 0.3f).toArgb(),
        ...
        moreKeysKeyboardBackground = kdcMatcher.matchMoreKeysKeyboardBackground("") ?: if (isPlume) {
            shadowedRoundedRectangle(colorScheme.keyboardContainer.toArgb(), dp(10.dp))
        } else coloredRoundedRectangle(colorScheme.keyboardPress.toArgb(), dp(keyCornerRadius))
