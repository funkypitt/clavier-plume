...
// Plume bar (docs/05-reglages-minimalistes.md §10): action key = Emojis; pinned = Voice Input;
// favorites = Language switch, Undo, Redo, Text Editor, Clipboard, Keyboard Modes;
// "All actions" = Cut, Copy, Paste, Select All, arrows, Theme Switcher, Settings;
// hidden = Debug Info, Voice Input (System), Bug Viewer, WordStyles.
val DefaultActionSettings = mapOf(
    ActionCategory.ActionKey to listOf(EmojiAction),
    ActionCategory.PinnedKey to listOf(SwitchLanguageAction, VoiceInputAction),   // Plume : bascule FR/EN en un tap
    ActionCategory.Favorites to listOf(UndoAction, RedoAction, TextEditAction, ClipboardHistoryAction, KeyboardModeAction),
    ActionCategory.More to listOf(), // Remaining actions get populated automatically by ensureWellFormed
    ActionCategory.Disabled to listOf(MemoryDebugAction, SystemVoiceInputAction, BugViewerAction, FontTyperAction)
)
