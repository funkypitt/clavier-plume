...
fun BlacklistScreen(navController: NavHostController = rememberNavController()) {
    ...
    ScrollableList {
        ScreenTitle(stringResource(R.string.plume_blacklist), showBack = true, navController)

        blockOffensiveWordsSetting.component()
        // Plume: the "block slurs" toggle (English only) is hidden, its default stays

        SectionHeader(stringResource(R.string.prediction_settings_word_blacklist_edit_blacklisted_words_title))
        Row(modifier = Modifier.padding(16.dp, 0.dp, 8.dp, 0.dp)) {
