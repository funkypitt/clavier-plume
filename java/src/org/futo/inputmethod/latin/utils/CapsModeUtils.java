...
public final class CapsModeUtils {
    ...
    public static int getCapsMode(final CharSequence cs, final int reqModes,
        ...
        // Plume : « etc. », « p. ex. », « M. », « cf. » ne terminent pas la phrase
        {
            int k = j;
            while (k > 0 && (Character.isLetter(cs.charAt(k - 1)) || cs.charAt(k - 1) == '°'
                    || (cs.charAt(k - 1) == '.' && k > 1 && Character.isLetter(cs.charAt(k - 2))))) {
                k--;
            }
            if (k < j && org.futo.inputmethod.latin.plume.PlumeRules.isAbbreviation(cs.subSequence(k, j))) {
                return noCaps;
            }
        }
        int state = START;
