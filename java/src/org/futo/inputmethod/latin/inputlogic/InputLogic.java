...
public final class InputLogic {
    ...
    public InputTransaction onPickSuggestionManually(final SettingsValues settingsValues,
        ...
        if(suggestionInfo.mKindAndFlags == SuggestedWordInfo.KIND_PLUME_REVERT) {
            ...
            // retour arrière qui annule, sans effacer quoi que ce soit d'autre.
            inputTransaction.setRequiresUpdateSuggestions();
            if (mLastComposedWord.canRevertCommit()) {
                final String typed = mLastComposedWord.mTypedWord;
                TypoLogger.log(typed, mLastComposedWord.mCommittedWord.toString(),
                        getDictionaryFacilitatorLocale().toString(), "revert");
                mConnection.beginBatchEdit();
                revertCommit(inputTransaction, settingsValues);
                mConnection.endBatchEdit();
                StatsUtils.onRevertAutoCorrect(mImeHelper.getContext(), typed);
            }
            return inputTransaction;
        }
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
        ...
        // Plume : guillemets français « » (avec insécables) à la place de "
        if (processedEvent.mCodePoint == Constants.CODE_DOUBLE_QUOTE && !processedEvent.isConsumed()) {
            final String quote = org.futo.inputmethod.latin.plume.PlumeRules.frenchQuote(
                    mConnection.getTextBeforeCursor(200, 0), settingsValues.mLocale,
                    plumeIsSensitiveField(settingsValues));
            if (quote != null) {
                if (mWordComposer.isComposingWord()) {
                    commitTyped(settingsValues, LastComposedWord.NOT_A_SEPARATOR);
                }
                mConnection.commitText(quote, 1);
                mSpaceState = SpaceState.NONE;
                inputTransaction.setRequiresUpdateSuggestions();
                inputTransaction.setDidAffectContents();
                mConnection.endBatchEdit();
    ...
    private void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction) {
        ...
        if (tryPerformDoubleSpacePeriod(event, inputTransaction)) {
            ...
            if(autoInsertSpaces
                ...
                // Plume : en français, espace fine insécable avant ; ! ? et insécable avant :
                final int spaceCp = org.futo.inputmethod.latin.plume.PlumeRules
                        .spaceBeforePunctuation(codePoint, settingsValues.mLocale);
                sendKeyCodePoint(settingsValues, spaceCp);
                mSpaceState = SpaceState.SYMBOL_PREFIX;

                if(inputTransaction.didAutoCorrect()) {
                    mLastComposedWord.mSeparatorString = new String(Character.toChars(spaceCp))
    ...
    private void handleBackspaceEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (mWordComposer.isComposingWord() && !mConnection.hasSelection()) {
            ...
            if (mLastComposedWord.canRevertCommit()
                ...
                // Plume/TypoLogger : correction refusée par l'utilisateur (faux positif à apprendre)
                TypoLogger.log(lastComposedWord, mLastComposedWord.mCommittedWord.toString(),
                        getDictionaryFacilitatorLocale().toString(), "revert");
    ...
    void unlearnWord(final String word, final SettingsValues settingsValues, final int eventType) {
        ...
        // Plume : un mot effacé au retour arrière ou dont la correction est annulée ne compte
        // pas comme une validation (sinon deux fautes identiques corrigées à la main suffiraient
        // à l'apprendre)
        org.futo.inputmethod.latin.plume.PlumeVocab.uncount(
                org.futo.inputmethod.latin.plume.PlumeVocab.canonical(word,
     ...
     * in ... ? ? ! ?
     */
    /** Plume : champs où la typographie (’ « ») ne doit pas s'appliquer. */
    private static boolean plumeIsSensitiveField(final SettingsValues settingsValues) {
        final InputAttributes a = settingsValues.mInputAttributes;
        return a.mIsPasswordField || a.mIsEmailField || a.mIsUriField || a.mIsCodeField
                || a.mIsNumericalPasswordField;
    }
    ...
    public boolean resetComposingWord(
        ...
        // Plume : le mot repris peut contenir une apostrophe typographique ; le dictionnaire est en '
        final String typedWordString = range.mWord.toString().replace('\u2019', '\'');
    ...
    private void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
        ...
        // Plume : apostrophe typographique dans le texte écrit (le dictionnaire reste en ')
        final CharSequence plumeCommitted = org.futo.inputmethod.latin.plume.PlumeRules.typographic(
                chosenWordWithSuggestions, locale, plumeIsSensitiveField(settingsValues));
        mConnection.commitText(plumeCommitted, 1);
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
        ...
        mLastComposedWord = mWordComposer.commitWord(commitType,
                plumeCommitted, separatorString, ngramContext);
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
