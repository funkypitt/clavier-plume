...
class IMEHelper(
    ...
    /** Plume : poids probabilistes des lettres suivantes (0..1) pour le hit-testing. */
    public fun updateBoostWeights(weights: Map<Int, Float>?) {
        latinIME.latinIMELegacy.mKeyboardSwitcher?.mainKeyboardView?.mKeyDetector?.let {
            it.updateBoostWeights(weights)
        }
    }
